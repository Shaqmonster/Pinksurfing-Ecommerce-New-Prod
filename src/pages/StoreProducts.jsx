...
export default function StoreProducts() {
    ...
    return shoppingProduct
      .filter((i) => {
        ...
        const categoryFilterReturn =
          categoryFilter === "all" ? true : i?.category?.name === categoryFilter;
    ...
    const getFilterProducts = async () => {
      ...
      try {
        if (filterBy === "") {
          ...
          setCategoryOnlyData(getUniqueData(response.data.Products, "category"));
          ...
          const response = await axios.get(
            `${import.meta.env.VITE_SERVER_URL}/api/product/filter-products/?attributes__value=${filterBy}`,
            { headers: { "Content-Type": "application/json" } }
          );
  ...
  useEffect(() => {
    if (shoppingProduct && shoppingProduct.length > 0) {
      const maxval = shoppingProduct.reduce((max, obj) => {
        return Math.max(max, Number(obj.unit_price) || 0);
      }, 0);
      setMaximumValue(maxval || 20000);
      setMaxValue(maxval || 20000);
    }
  }, [shoppingProduct]);
  ...
  uniqueAttributes.forEach((attribute) => {
    ...
    switch (name) {
      case "color":
        if (!colorArray.includes(value)) colorArray.push(value);
        break;
      case "storage":
        if (!storageArray.includes(value)) storageArray.push(value);
        break;
      case "size":
        if (!sizeArray.includes(value)) sizeArray.push(value);
        break;
      case "os":
      case "system":
        if (!systemArray.includes(value)) systemArray.push(value);
        break;
    }
  });
                ...
                <div className="p-4 space-y-6">
                  ...
                  <div className="bg-gray-800 p-4 rounded-2xl border border-gray-700">
                    ...
                    <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                      {CategoryOnlyData.filter(cat => cat && cat !== 'null' && cat !== 'undefined').map((category, index) => (
                        <button
                          key={index}
                          onClick={() => {
      ...
      <main className="relative z-10 max-w-[1600px] mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6">
        ...
        <div className="mb-5">
          ...
          <nav className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 mb-4">
            ...
            <Link to="/shoppingMall" className="hover:text-purple-600 dark:hover:text-purple-400 transition-colors">Stores</Link>
            <ChevronRightIcon className="w-4 h-4" />
            <span className="text-purple-600 dark:text-purple-400 font-medium capitalize">{slug?.replace(/-/g, ' ')}</span>
          </nav>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-purple-600 to-pink-600 rounded-xl flex items-center justify-center shadow-lg">
                  <span className="text-white font-bold text-lg sm:text-xl">{slug?.charAt(0).toUpperCase()}</span>
                </div>
                <div>
          ...
          <div className="lg:col-span-4 xl:col-span-5">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center min-h-[500px] glass-card rounded-3xl">
                ...
                <p className="mt-6 text-lg text-gray-500 dark:text-gray-400 animate-pulse">Loading amazing products...</p>
              </div>
            ) : filteredProducts.length === 0 ? (
              /* Empty State */
              <div className="flex flex-col items-center justify-center min-h-[500px] glass-card rounded-3xl p-8">
                <div className="w-32 h-32 bg-gradient-to-br from-purple-100 to-pink-100 dark:from-purple-900/30 dark:to-pink-900/30 rounded-full flex items-center justify-center mb-6 animate-bounce-subtle">
