...
const SORT_OPTIONS = [
  ...
  { value: "-total_orders_completed", label: "Most Popular" },
];
...
const GigsPage = () => {
  ...
  useEffect(() => { getGigCategories().then((res) => setCategories(Array.isArray(res.data) ? res.data : res.data.results || [])).catch(() => {}); }, []);
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-10">
          ...
          <h1 className="text-3xl sm:text-5xl font-bold text-white mb-3">
            Find the perfect{" "}
            <span className="bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">freelance service</span>
          </h1>
          ...
          <div className="mt-6 flex flex-col sm:flex-row items-center gap-3 max-w-2xl mx-auto">
            ...
            <motion.button
              ...
              className="px-6 py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-500 text-white font-semibold text-sm shadow-lg hover:shadow-purple-500/30 transition-all whitespace-nowrap"
            >
              Search
            </motion.button>
            <motion.button
            ...
            >
              ...
              Filters
              {hasActiveFilters && (
                <span className="bg-pink-500 text-white text-[10px] rounded-full w-4 h-4 flex items-center justify-center">!</span>
              )}
            </motion.button>
          </div>
        </motion.div>
              ...
              <div className="bg-[#13131a] border border-white/5 rounded-2xl p-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="number" min="0" value={filters.min_price}
                    onChange={(e) => applyFilter("min_price", e.target.value)} placeholder="0"
                ...
                <div className="space-y-1">
                  ...
                  <input type="number" min="0" value={filters.max_price}
                    onChange={(e) => applyFilter("max_price", e.target.value)} placeholder="Any"
                    className="w-full bg-[#1a1a24] border border-white/10 rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-purple-500 transition-all" />
                </div>
                <div className="space-y-1">
                  <label className="text-white/50 text-xs">Category</label>
                  <select value={filters.category_slug} onChange={(e) => applyFilter("category_slug", e.target.value)}
                ...
                <div className="space-y-1">
                  ...
                  <select value={filters.min_rating} onChange={(e) => applyFilter("min_rating", e.target.value)}
                    className="w-full bg-[#1a1a24] border border-white/10 rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-purple-500 transition-all [&>option]:bg-[#1a1a24]">
                ...
                <div className="space-y-1">
                  ...
                  <select value={filters.delivery_days} onChange={(e) => applyFilter("delivery_days", e.target.value)}
                    className="w-full bg-[#1a1a24] border border-white/10 rounded-lg px-3 py-2 text-white text-sm outline-none focus:border-purple-500 transition-all [&>option]:bg-[#1a1a24]">
                    <option value="">Any</option>
                    {[1, 3, 7, 14, 30].map((d) => <option key={d} value={d}>Up to {d} days</option>)}
                  </select>
                </div>
              </div>
              {hasActiveFilters && (
        ...
        <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {filters.search && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 bg-purple-500/10 border border-purple-500/20 rounded-full text-purple-400 text-xs">
                "{filters.search}"
                <button onClick={() => { setSearchQuery(""); applyFilter("search", ""); }}
                  className="ml-1 hover:text-white transition-colors"><IoCloseOutline className="text-sm" /></button>
              </span>
            )}
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <IoSwapVerticalOutline className="text-white/40 text-sm" />
              <select
                value={filters.ordering}
                onChange={(e) => applyFilter("ordering", e.target.value)}
                className="bg-[#13131a] border border-white/10 rounded-lg px-3 py-1.5 text-white/70 text-xs outline-none focus:border-purple-500 transition-all [&>option]:bg-[#1a1a24]"
              >
